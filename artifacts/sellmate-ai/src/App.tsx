...
function renderMarkdownInline(value: string): ReactNode[] {
  return value.split(/(\*\*[^*]+\*\*|`[^`]+`|\*[^*]+\*)/g).map((part, index) => {
    ...
    if (part.startsWith('`') && part.endsWith('`')) {
      return <code key={`${part}-${index}`} className="rounded bg-background/70 px-1 py-0.5 text-[.9em]">{part.slice(1, -1)}</code>;
    }
    if (part.startsWith('*') && part.endsWith('*')) {
...
function MarkdownText({ content }: { content: string }) {
  return <p className="whitespace-pre-wrap">{content.split(/\r?\n/).map((line, index, lines) => <span key={`${line}-${index}`}>{renderMarkdownInline(line)}{index < lines.length - 1 && <br />}</span>)}</p>;
}
...
function AssistantPage() {
  ...
  useEffect(() => {
    const instructionText = settings.aiInstructions.trim();
